...
int main(int argc, char **argv)
{
  ...
  while(ros::ok())
  {
	    ...
	    float r1 = a + k1 * dot(diff, v1) / sqrt(distancesq);
		r = r1 + k2 * dot(diff, vg) / sqrt(distancesq);
		...
		/*if(modV1 < 0.001 && abs(v1.z) < 0.001 && f<r1 && f>0.1)
		{	
			coords.pose.position.x=p1.x-(p1.x*(r1+0.3))/sqrt(p1.x*p1.x + p1.y*p1.y);
			coords.pose.position.y=p1.y-(p1.y*(r1+0.3))/sqrt(p1.x*p1.x + p1.y*p1.y);
		...
		{  
			...
			if (modV1 < 0.001 && abs(v1.z) > 0.001)			//For attacks done along z axis only
			{
			...
			else if(modV1  < 0.01 && abs(v1.z) < 0.001)
			{
				...
				if(abs(p2.x - t1) > 0.01 || abs(p2.y - t2) > 0.01){
				ROS_INFO_STREAM("Hello");
				ROS_INFO_STREAM(v1);
				coords.pose.position.x=p2.x-(diff.y*(a))/R;
		    	coords.pose.position.y=p2.y+(diff.x*(a))/R;	
		    	}
		    	else if(f < a){
		    		coords.pose.position.x=p1.x-(p1.x*(r1+0.3))/sqrt(p1.x*p1.x + p1.y*p1.y);
					coords.pose.position.y=p1.y-(p1.y*(r1+0.3))/sqrt(p1.x*p1.x + p1.y*p1.y);
